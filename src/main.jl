...
function main(P)
    ...
    K1 = deepcopy(Ksparse)   # K original
    K2 = deepcopy(Ksparse)   # K original
    K3 = deepcopy(Ksparse)   # K original
    K4 = deepcopy(Ksparse)   # K original
    K5 = deepcopy(Ksparse)   # K original
    K6 = deepcopy(Ksparse)   # K original
    K7 = deepcopy(Ksparse)   # K original
    K8 = deepcopy(Ksparse)   # K original
    K9 = deepcopy(Ksparse)   # K original
    K10 = deepcopy(Ksparse)   # K original

    alphaa_t = 0.995
    # alphaa = 1.00
    for id in did
#=         K1[id] = alphaa_t .* Korig[id]
        K2[id] = (alphaa_t^2) .* Korig[id] 
        K3[id] = (alphaa_t^3) .* Korig[id] 
        K4[id] = (alphaa_t^4) .* Korig[id] 
        K5[id] = (alphaa_t^5) .* Korig[id] 
        K6[id] = (alphaa_t^6) .* Korig[id] 
        K7[id] = (alphaa_t^7) .* Korig[id] 
        K8[id] = (alphaa_t^8) .* Korig[id] 
        K9[id] = (alphaa_t^9) .* Korig[id]  =#
        K10[id] = (alphaa_t) .* Korig[id] 
    end
    kt = 0 # new iterator 
    ...
    open(string(out_dir,"Vfbefore.out"), "w") do Vfbefore
    open(string(out_dir,"Vfafter.out"), "w") do Vfafter
...
#=               if alphaa <= 1.00 && it > 3
        ...
        # Imposed Precursors
#=         if Vfmax > 1.0e-7 && slipstart2 == 0 && t > 3.0 * P[1].yr2sec && alphaa >= 1.0 
            #---------------
            # vs rigidity change
            # --------------
            print("\n\nAlpha reduction here\n")
...
#=         if Vfmax <= 1e-7 && amax <= 0 && slipstart2 == 1 && isolver == 1
        #if amax <= 0 && slipstart2 == 1 && isolver == 1 && Vfmax <= 1e-5
        ...
        if Vfmax > 1.01*P[2].Vthres && slipstart == 0
            ...
            write(Vfbefore, join(2*v[P[4].iFlt] .+ P[2].Vpl, " "), "\n")
        ...
        if Vfmax < 0.99*P[2].Vthres && slipstart == 1
            ...
            write(Vfafter, join(2*v[P[4].iFlt] .+ P[2].Vpl, " "), "\n")
...
#=             if slipstart2 == 1
        ...
        if isolver == 1 && Vfmax < 5e-3 || isolver == 2 && Vfmax < 1e-3
